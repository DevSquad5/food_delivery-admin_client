...
export async function RegistrationRequest(email,firstName,lastName,mobile,password,photo){
    try {
        store.dispatch(ShowLoader())
        let URL=BaseURL+"/registration/admin";
        let PostBody={email:email,firstName:firstName,lastName:lastName,phoneNo:mobile,password:password, photo:photo}
        let res=await axios.post(URL,PostBody)
        store.dispatch(HideLoader())
        if(res.status===200){
...
export async function ProfileUpdateRequest(email,firstName,lastName,mobile,password,photo){
    try {
        store.dispatch(ShowLoader())
        let URL=BaseURL+"/updateProfile/admin";
        let PostBody={email:email,firstName:firstName,lastName:lastName,phoneNo:mobile,password:password,photo:photo}
        let UserDetails={email:email,firstName:firstName,lastName:lastName,phoneNo:mobile,photo:photo};
